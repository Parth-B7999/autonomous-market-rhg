...
# break-even prices a_i = r_H2 * eta_i * 1e3 [$/MWh]; all six agents now differ, so the
# figures show the BAND rather than a single line
A_LO = min(R._a(i) for i in range(N)) * 1e3
A_HI = max(R._a(i) for i in range(N)) * 1e3
...
for c, (wk, rows) in enumerate(data.items()):
    ...
    axes[0, c].axhspan(A_LO, A_HI, color="r", alpha=0.12, lw=0)
    axes[0, c].axhline(A_LO, color="r", ls=":", lw=0.8,
                       label=f"H$_2$ break-even range \\${A_LO:.0f}--\\${A_HI:.0f}")
    axes[0, c].axhline(A_HI, color="r", ls=":", lw=0.8)
    ...
    axes[1, c].axhspan(A_LO, A_HI, color="r", alpha=0.12, lw=0)
    axes[1, c].axhline(A_LO, color="r", ls=":", lw=0.8,
                       label=f"H$_2$ break-even range \\${A_LO:.0f}--\\${A_HI:.0f}")
    axes[1, c].axhline(A_HI, color="r", ls=":", lw=0.8)
...
t = np.arange(96) * 0.25
fig, axes = plt.subplots(3, 3, figsize=(15, 11))   # no sharex: mechanism panels keep hour ticks
# all six agents are distinct now — every one gets its own panel
order = list(range(N))
_TY = {"grid": "grid", "solar": "solar", "wind": "wind"}
titles = [f"{names[i]} ({_TY[R._typ(i)]}, $P_{{max}}$={R._pmax(i):.0f} kW, $\\eta$={R._eta(i):.4f})"
          for i in order]
panel = [(0,0),(0,1),(0,2),(1,0),(1,1),(1,2)]
for (rr,cc), i, ttl in zip(panel, order, titles):
    ...
    # farm cap line: agents whose farm exceeds their stack are FORCED to curtail at peak
    if R._is_ren(i) and R._rcap(i) > pmax:
        ax.axhline(R._rcap(i), color=C_CURT, ls="-.", lw=1,
                   label="Farm cap (> stack $\\Rightarrow$ forced curtail)")
    ax.set_title(ttl, fontsize=9); ax.set_xlim(0, 24); ax.set_xticks([0,6,12,18,24])
    ax.set_ylim(0, max(pmax, R._rcap(i))*1.15)
    if cc == 0: ax.set_ylabel("power [kW]")
    if rr == 1: ax.set_xlabel("hour of day")
# aggregate panel + FACET accuracy on the bottom row
axes[2,0].plot(t, r["P"].sum(1), color="#1f6feb", lw=1, label="$\\Sigma p$")
axes[2,0].axhline(LMAX, color="purple", ls="--", lw=1); axes[2,0].axhline(LMIN, color="green", ls="--", lw=1)
axes[2,0].set_title(f"Coalition $\\Sigma p$ ({WEEKS[wk][DAYIDX]})"); axes[2,0].set_ylim(0, LMAX*1.08); axes[2,0].set_ylabel("kW")
axes[2,0].set_xlim(0, 24); axes[2,0].set_xticks([0,6,12,18,24]); axes[2,0].set_xlabel("hour of day")
...
handles = [Patch(fc=C_REN,label="Renewable used"), Patch(fc=C_GRID,label="Grid import"),
           Patch(fc="none",ec=C_CURT,hatch="////",label="Curtailed"),
           Line2D([],[],color="k",lw=1.1,label="Converter load"),
           Line2D([],[],color="0.35",ls=":",lw=1,label="$P_{max}$ (stack)"),
           Line2D([],[],color=C_CURT,ls="-.",lw=1,label="Farm cap (forced curtail)")]
axes[2,2].axis("off")
axes[2,2].legend(handles=handles, loc="center", fontsize=10, frameon=False)
fig.suptitle(f"Per-agent power balance, all six distinct agents, {WEEKS[wk][DAYIDX]} "
             f"(evening retreat to $L_{{min}}$=100 kW) + FACET accuracy", y=0.995)
fig.tight_layout(rect=[0,0,1,0.98]); fig.savefig(OUT/"fig4_mechanism.pdf"); fig.savefig(OUT/"fig4_mechanism.png"); plt.close(fig)
