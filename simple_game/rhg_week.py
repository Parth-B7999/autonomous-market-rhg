...
def theta_pgne(D, lam4, p_da, cs4, cw4):
    ...
    th[PDA0:PDA0 + N] = p_da
    # farm size is shared within a type (see rhg_mpqp.type_rcap) — never hardcode it here
    th[GS0:GS0 + H] = np.clip(cs4, 0, 1) * R.type_rcap("solar")    # solar output = CF × PV farm
    th[GW0:GW0 + H] = np.clip(cw4, 0, 0.8) * R.type_rcap("wind")   # wind output  = CF × wind farm
    return th
...
def run_day(day, sols, game, rng, verbose=True):
    ...
    p_da, dam_res = F.solve_dam_admm(lam_da_fc, cs_fc, cw_fc, d_day)   # (N,24) distributed DAM
    t_dam = time.perf_counter() - t_dam
    dam_iters = int(getattr(dam_res, "n_iter", -1))
    dam_conv = bool(getattr(dam_res, "converged", False))
    ...
    t_step = np.zeros(96)          # per-step RTM clearing wall time [s]
    fb_step = np.zeros(96, bool)   # did this step fall back to iterative ADMM?
    # The exact θ presented to solve_step at each step.  Recorded (not re-derived) so that
    # bench_solvers.py can replay the IDENTICAL parameter sequence to the baseline solvers:
    # θ depends on h2, which depends on the dispatch, so re-deriving it elsewhere would drift.
    th_step = np.zeros((96, game.n_p))
    for t in range(96):
        if t % 20 == 0:
        ...
        dt_step = time.perf_counter() - s
        prev_x = x
        t_step[t] = dt_step
        fb_step[t] = comm.get("fallback", 0) > fb_before
        t_on += dt_step
    ...
    return dict(day=day, h2=h2, d_day=d_day, P=P, e_cent=e_cent, n_cent=n_cent,
                ...
                t_step=t_step, fb_step=fb_step, th_step=th_step,
                dam_iters=dam_iters, dam_conv=dam_conv)
